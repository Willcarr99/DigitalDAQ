...
void v1730DPP_ReadAndApplySettings(MVME_INTERFACE *mvme, uint32_t base){
  ...
  std::ifstream f("settings-DPP.dat");
  if (!f){
    // Print an error and exit
    std::cerr << "ERROR! settings-DPP.dat could not be opened" << std::endl;
    exit(1);
  }
  ...
  std::cout << "Long gate = " << tlong << std::endl;
  std::cout << "Short gate = " << tshort << std::endl;
  std::cout << "Gate offset = " << toffset << std::endl;
  std::cout << "Trigger Hold-Off = " << trigHoldOff << std::endl;
  std::cout << "Pre-Trigger = " << preTrig << std::endl;
  std::cout << "Gain = " << cGain << std::endl;
  std::cout << "negSignals = " << negSignals << std::endl;
  std::cout << "offset = " << offset << std::endl;
  std::cout << "cThresh = " << cThresh << std::endl;
  std::cout << "cCFDDelay = " << cCFDDelay << std::endl;
  std::cout << "cCFDFraction = " << cCFDFraction << std::endl;
  std::cout << "Dynamic Range = " << dRange << std::endl;
  std::cout << "Input Smoothing = " << inputSmoothing << std::endl;
  ...
  std::cout << "Charge Zero Suppression Threshold = " << chargeThresh << std::endl;
  std::cout << std::endl << "--------------------------------------------------" << std::endl;

  // Is the digitizer working?
  v1730DPP_getFirmwareRev(mvme, base);

  // Reset the module
  std::cout << "Reset module" << std::endl;
  v1730DPP_SoftReset(mvme, base);
  
  sleep(1);
  
  // General setup
  v1730DPP_RegisterWrite(mvme, base, 0x8000, 0x800C0110); // 0x800C0110 - Board Config
  //v1730DPP_RegisterWrite(mvme, base, 0x8084, 0x); // DPP Algorithm Control
  v1730DPP_setAggregateOrg(mvme, base, 3);
  v1730DPP_setAggregateSizeG(mvme, base, 1023);   // 1023 events per aggregate
  v1730DPP_setRecordLengthG(mvme, base, 64);    // Don't collect waveforms
  v1730DPP_setDynamicRangeG(mvme, base, dRange);    //(0 = 2 Vpp)  (1 = 0.5 Vpp)
  
  for(int i=0; i<15; i++){
    v1730DPP_DisableChannel(mvme, base, i);
  }
  ...
  for(int i=0; i<15; i++){
    ...
    v1730DPP_EnableChannel(mvme, base, enableCh[i]);
  }
  
  // Gains
  v1730DPP_setGainG(mvme, base, cGain);
  v1730DPP_setInputSmoothingG(mvme, base, inputSmoothing); // 0 - disabled, 1 - 2 samples, 2 - 4 samples, 3 - 8 samples, 4 - 16 samples
  v1730DPP_setPolarityG(mvme, base, negSignals); // 0 for positive, 1 for negative
  //v1730DPP_setPolarity(mvme, base, negSignals, 0);
  //v1730DPP_setPolarity(mvme, base, 0, 1);
  v1730DPP_setMeanBaselineCalcG(mvme, base, meanBaseline); // 0 = fixed, 1 = 16 samples, 2 = 64, 3 = 256, 4 = 1024
  v1730DPP_setFixedBaselineG(mvme, base, fixedBaseline); // 0 = disabled
  v1730DPP_setBaselineCalcRestartG(mvme, base, restartBaseline); // 0 - disabled (default), 1 - enabled
  v1730DPP_setOppositePolarityDetectionG(mvme, base, oppPol); // 0 for enabled (default), 1 for disabled
  v1730DPP_setChargeZeroSuppressionThresholdG(mvme, base, chargeThresh); // 0 = disabled

  // Gates and triggers
  v1730DPP_setCFDG(mvme, base, cCFDDelay, cCFDFraction);
  v1730DPP_setLongGateG(mvme, base, tlong);
  v1730DPP_setShortGateG(mvme, base, tshort);
  v1730DPP_setGateOffsetG(mvme, base, toffset);
  v1730DPP_setTriggerHoldoffG(mvme, base, trigHoldOff);
